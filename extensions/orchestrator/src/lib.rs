...
use crate::paths::{issue_workspace, issue_workspace_path, workflow_key, AgentPaths};
...
use crate::store::{
    new_run_id, NewClaim, NewEvent, NewHeartbeat, NewRun, RunFinish, Store,
...
impl Extension for OrchestratorExtension {
    ...
    fn start<'a>(&'a self, ctx: host_api::StartCtx) -> host_api::BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let root = ctx.paths.root().to_path_buf();
            let workflow_root = ctx.paths.workflow_root().to_path_buf();
            let paths = if workflow_root == root {
                ...
                AgentPaths::with_workflow(root, workflow_root, state_dir)
            };
            std::fs::create_dir_all(paths.logs_dir())
                .with_context(|| format!("creating logs dir {}", paths.logs_dir().display()))?;
            let log_guard = logging::init(&paths.log_file())?;
            ...
            // Passive agent: no WORKFLOW.md for this workflow → no loop.
            // Boot on runner + foreground + extensions alone; skip the tracker
            // build, runner resolve and the tick loop entirely.
            if !paths.workflow_md().exists() {
            ...
            prompt
                ...
                .frontmatter
                .validate_loop()
                .context("incomplete WORKFLOW.md loop config")?;
            let effective_cfg =
                EffectiveLoopConfig::resolve(&agent_cfg, &prompt.snapshot().frontmatter);
            ...
            let mut api_control_rx = ctx
                .host
                .bus
                .subscribe::<orchestrator_api::ControlMsg>(CONTROL_TOPIC)?;
            let mut run_requested_rx = ctx
                .host
                .bus
                .subscribe::<RunRequested>(RUN_REQUESTED_TOPIC)?;
            let mut dispatch_requested_rx = ctx
                .host
                .bus
                .subscribe::<DispatchRequested>(DISPATCH_REQUESTED_TOPIC)?;
            let bridge_tx = control_tx.clone();
            tokio::spawn(async move {
                while let Ok(msg) = api_control_rx.recv().await {
                    bridge_control_msg(&bridge_tx, msg).await;
                }
            });
            let run_request_tx = control_tx.clone();
            tokio::spawn(async move {
                while let Ok(msg) = run_requested_rx.recv().await {
                    bridge_request_msg(&run_request_tx, msg.identifier);
                }
            });
            let dispatch_request_tx = control_tx.clone();
            tokio::spawn(async move {
                while let Ok(msg) = dispatch_requested_rx.recv().await {
                    bridge_request_msg(&dispatch_request_tx, msg.identifier);
                }
            });
...
fn bridge_request_msg(tx: &tokio::sync::mpsc::UnboundedSender<ControlMsg>, identifier: String) {
    let (reply, _rx) = tokio::sync::oneshot::channel();
    let _ = tx.send(ControlMsg::Claim { identifier, reply });
}

async fn bridge_control_msg(
    tx: &tokio::sync::mpsc::UnboundedSender<ControlMsg>,
    msg: orchestrator_api::ControlMsg,
) {
    match msg {
        orchestrator_api::ControlMsg::Stop => {
            let _ = tx.send(ControlMsg::Stop);
        }
        orchestrator_api::ControlMsg::Pause => {
            let _ = tx.send(ControlMsg::Pause);
        }
        orchestrator_api::ControlMsg::Resume => {
            let _ = tx.send(ControlMsg::Resume);
        }
        orchestrator_api::ControlMsg::Tick { reply } => {
            let (local_tx, local_rx) = tokio::sync::oneshot::channel();
            if tx.send(ControlMsg::Tick { reply: local_tx }).is_err() {
        ...
        orchestrator_api::ControlMsg::Claim { identifier, reply } => {
            let (local_tx, local_rx) = tokio::sync::oneshot::channel();
            if tx
                .send(ControlMsg::Claim {
                    identifier,
                    reply: local_tx,
                })
                .is_err()
        ...
        orchestrator_api::ControlMsg::Release { run_id, reply } => {
            let (local_tx, local_rx) = tokio::sync::oneshot::channel();
            if tx
                .send(ControlMsg::Release {
                    run_id,
                    reply: local_tx,
                })
                .is_err()
        ...
        orchestrator_api::ControlMsg::Interrupt { run_id, reply } => {
            let (local_tx, local_rx) = tokio::sync::oneshot::channel();
            if tx
                .send(ControlMsg::Interrupt {
                    run_id,
                    reply: local_tx,
                })
                .is_err()
        ...
        orchestrator_api::ControlMsg::Kill { run_id, reply } => {
            let (local_tx, local_rx) = tokio::sync::oneshot::channel();
            if tx
...
async fn forward_reply(
    reply: orchestrator_api::ControlReplyTx,
    rx: tokio::sync::oneshot::Receiver<ControlReply>,
) {
    match rx.await {
        Ok(value) => orchestrator_api::send_reply(
            &reply,
            orchestrator_api::ControlReply {
                ok: value.ok,
                message: value.message,
            },
        ),
        Err(_) => orchestrator_api::send_reply(
            &reply,
            orchestrator_api::ControlReply::err("orchestrator unavailable"),
        ),
    }
...
impl Orchestrator {
    ...
    fn maybe_reload_workflow(&mut self) {
        match self.prompt.maybe_reload() {
            Ok(false) => {}
            Ok(true) => {
                let new_eff = EffectiveLoopConfig::resolve(
                    &self.agent_cfg,
    ...
    fn maybe_reload_agent_config(&mut self) {
        ...
        let new_eff =
            EffectiveLoopConfig::resolve(&self.agent_cfg, &self.prompt.snapshot().frontmatter);
    ...
    fn apply_effective_config(
        ...
        let tracker_changed = new_eff.active_states != self.effective_cfg.active_states
            ...
            || new_eff.tracker_projects != self.effective_cfg.tracker_projects
            || new_eff.tracker_config_path != self.effective_cfg.tracker_config_path
        ...
        if tracker_changed {
            // Tracker dimensions are resolved entirely from the (frontmatter)
            // loop config; rebuild from the new resolved config.
            match tracker::build_configured(
                &self.runner_services,
                &new_eff,
                self.paths.workflow_root.clone(),
            ) {
                Ok(t) => {
                    self.tracker = t;
                    logging::ev(
                ...
                Err(e) => {
                    ...
                    new_eff.tracker_projects = self.effective_cfg.tracker_projects.clone();
                    new_eff.tracker_config_path = self.effective_cfg.tracker_config_path.clone();
    ...
    async fn try_dispatch(&mut self, issue: Issue, attempt: u32) {
        ...
        let ws_root = self
            .paths
            .workspace_root(&self.effective_cfg.workspace_root);
    ...
    async fn publish_snapshots(&self, candidates: &[Issue]) {
        ...
        if let Some(bus) = &self.snapshot_bus {
            let _ = bus.publish(RUN_SNAPSHOT_TOPIC, self.snapshot().await);
        }
    }
    ...
    async fn snapshot(&self) -> RunSnapshot {
        RunSnapshot {
            ...
            queue: self
                ...
                .clone()
                .into_iter()
                .map(|item| orchestrator_api::QueueItem {
            ...
            retry: self
                ...
                .clone()
                .into_iter()
                .map(|item| orchestrator_api::RetryItem {
                    identifier: item.identifier,
                ...
                .collect(),
            events: self.state.events.snapshot(),
            history: self
                .state
                .history
                .snapshot()
                .into_iter()
                .map(|entry| orchestrator_api::HistoryEntry {
                    identifier: entry.identifier,
                    status: api_status(entry.status),
                    pid: entry.pid,
            ...
            runs: self
                ...
                .map(api_run_row)
                .collect(),
            last_tick_at: *self.state.last_tick_at.read().await,
            rate_limit_min_remaining: {
                let value = self
                    .state
                    .rate_limit_min_remaining
            ...
            version: *self.state.version_tx.borrow(),
        }
    }
...
fn api_active_run(run: ActiveRun) -> orchestrator_api::ActiveRun {
    orchestrator_api::ActiveRun {
        run_id: run.run_id,
...
fn api_run_row(row: store::RunRow) -> orchestrator_api::RunRow {
    orchestrator_api::RunRow {
        run_id: row.run_id,
...
fn api_status(status: RunStatus) -> orchestrator_api::RunStatus {
    match status {
        RunStatus::Running => orchestrator_api::RunStatus::Running,
        RunStatus::RetryQueued => orchestrator_api::RunStatus::RetryQueued,
        RunStatus::Cancelled => orchestrator_api::RunStatus::Cancelled,
        RunStatus::Failed => orchestrator_api::RunStatus::Failed,
        RunStatus::Succeeded => orchestrator_api::RunStatus::Succeeded,
        RunStatus::Interrupted => orchestrator_api::RunStatus::Interrupted,
        RunStatus::Crashed => orchestrator_api::RunStatus::Crashed,
        RunStatus::NeedsHuman => orchestrator_api::RunStatus::NeedsHuman,
        RunStatus::Stalled => orchestrator_api::RunStatus::Stalled,
        RunStatus::Terminal => orchestrator_api::RunStatus::Terminal,
        RunStatus::HookFailed => orchestrator_api::RunStatus::HookFailed,
        RunStatus::DispatchFailed => orchestrator_api::RunStatus::DispatchFailed,
        RunStatus::Released => orchestrator_api::RunStatus::Released,
        RunStatus::Orphaned => orchestrator_api::RunStatus::Orphaned,
        RunStatus::ParkBarrier => orchestrator_api::RunStatus::ParkBarrier,
        RunStatus::Killed => orchestrator_api::RunStatus::Killed,
    }
}
...
fn default_runner_services() -> host_api::ServiceRegistry {
    ...
    let runners: [(&str, Arc<dyn cap_runner::Runner>); 4] = [
...
mod tests {
    ...
    use crate::workflow_config::{parse_workflow_md, EffectiveLoopConfig, WorkflowFrontmatter};

    /// WORKFLOW.md frontmatter carrying the loop config the orchestrator tests
    /// rely on (files tracker, todo/done, fast polling). Loop config now lives
    /// in frontmatter, not agent.yaml.
    const TEST_WF_FM: &str = "---\ntracker:\n  kind: files\n  path: issues\n  active_states: [todo]\n  terminal_states: [done]\npolling:\n  interval_ms: 100\n  max_concurrent: 1\n  max_retries: 3\n  retry_backoff_ms: 1000\nworkspace:\n  root: workspaces\n---\n";

    fn test_frontmatter() -> WorkflowFrontmatter {
        parse_workflow_md(TEST_WF_FM).unwrap().frontmatter
    }

    /// Write a WORKFLOW.md carrying `TEST_WF_FM` frontmatter plus `body`, so a
    /// live reload re-derives the same loop config from the file.
    fn write_workflow_md(root: &Path, body: &str) {
        std::fs::write(root.join("WORKFLOW.md"), format!("{TEST_WF_FM}{body}")).unwrap();
    }
    use anyhow::Result;
    use chrono::TimeZone;
    ...
    fn passive_snapshot_contains_agent_runner_identity() {
        ...
        assert_eq!(snapshot.agent.workflow, None);
        assert_eq!(snapshot.version, 1);
    }

    #[test]
    fn passive_snapshot_carries_non_default_workflow_label() {
        let cfg = AgentConfig {
            id: "agent-1".to_string(),
            name: "Agent 1".to_string(),
            runner: RunnerConfig {
        ...
        let snapshot =
            super::passive_snapshot(&cfg, "/tmp/agent".to_string(), Some("triage".to_string()));

        assert_eq!(snapshot.agent.workflow.as_deref(), Some("triage"));
    }
    ...
    /// Serialize the identity/host-only agent.yaml (no loop config, which lives
    /// in WORKFLOW.md now).
...
{}",
    ...
    #[tokio::test]
    async fn tick_reloads_agent_yaml_live_runner_fields() {
        let temp = TempDir::new().unwrap();
        write_workflow_md(temp.path(), "Do {{ issue.title }}");
        ...
        // Loop timing comes from WORKFLOW.md (unchanged); runner fields are the
        // live-reloadable agent.yaml surface.
        agent_cfg.runner.max_run_timeout_ms = 777;
        agent_cfg.runner.stall_timeout_ms = 888;
        write_agent_yaml(temp.path(), &agent_cfg);
        orchestrator.agent_config_reloader.mark_stale_for_test();

        orchestrator.tick().await;

        assert_eq!(orchestrator.effective_cfg.max_run_timeout_ms, 777);
        assert_eq!(orchestrator.effective_cfg.stall_timeout_ms, 888);
    ...
    async fn unknown_runner_in_agent_yaml_reverts_runner_but_keeps_other_live_fields() {
        ...
        // Loop timing stays sourced from WORKFLOW.md across the reload.
        assert_eq!(orchestrator.effective_cfg.poll_interval_ms, 100);
    }
    ...
    async fn boot_only_agent_yaml_fields_are_frozen() {
        ...
        // workspace_root comes from WORKFLOW.md frontmatter, unchanged here.
        assert_eq!(
    ...
    /// WORKFLOW.md is the sole source of loop timing: an agent.yaml reload
    /// (here a runner field) leaves the frontmatter-sourced poll interval intact.
    #[tokio::test]
    async fn workflow_frontmatter_polling_survives_agent_yaml_reload() {
        let temp = TempDir::new().unwrap();
        std::fs::write(
            temp.path().join("WORKFLOW.md"),
            "---\ntracker:\n  kind: files\n  path: issues\n  active_states: [todo]\n  terminal_states: [done]\npolling:\n  interval_ms: 222\nworkspace:\n  root: workspaces\n---\nDo {{ issue.title }}",
        ...
        agent_cfg.runner.max_run_timeout_ms = 999;
        write_agent_yaml(temp.path(), &agent_cfg);
        orchestrator.agent_config_reloader.mark_stale_for_test();

        orchestrator.tick().await;

        assert_eq!(orchestrator.agent_cfg.runner.max_run_timeout_ms, 999);
    ...
    #[tokio::test]
    async fn retained_snapshot_late_subscriber_gets_current_state() {
        ...
        snapshot.version = 7;
        ctx.bus
            .publish(orchestrator_api::RUN_SNAPSHOT_TOPIC, snapshot)
            .unwrap();

        let late = ctx
            .bus
            .subscribe_retained::<orchestrator_api::RunSnapshot>(
                orchestrator_api::RUN_SNAPSHOT_TOPIC,
            )
            .unwrap();
        assert_eq!(late.borrow().agent.id, "agent-a");
        assert_eq!(late.borrow().version, 7);
    }
    ...
    /// Frontmatter for `test_config`-based dispatch tests: fast retry cadence
    /// (max_retries 1, 10ms backoff) matching the retired agent.yaml trio.
    fn test_config_frontmatter() -> WorkflowFrontmatter {
        parse_workflow_md(
            "---\ntracker:\n  kind: files\n  path: issues\n  active_states: [todo]\n  terminal_states: [done]\npolling:\n  interval_ms: 10\n  max_concurrent: 1\n  max_retries: 1\n  retry_backoff_ms: 10\nworkspace:\n  root: workspaces\n---\n",
        )
        .unwrap()
    ...
    async fn control_and_shutdown_lifecycle_events_are_queryable() {
        ...
        let effective_cfg = EffectiveLoopConfig::resolve(&cfg, &test_config_frontmatter());
    ...
    #[tokio::test]
    async fn snapshot_carries_non_default_workflow_label() {
        let dir = tempdir().unwrap();
        let store = Arc::new(Store::open(&dir.path().join("store.db")).unwrap());
        let (control_tx, control_rx) = mpsc::unbounded_channel();
        let state = AppState::new(
            AgentInfo {
                ...
                tracker: "files".to_string(),
                runner: "fake".to_string(),
                workflow: Some("triage".to_string()),
            },
            control_tx,
            Arc::clone(&store),
            Vec::new(),
        ...
        let cfg = test_config("/usr/bin/true".to_string());
        let root = dir.path().canonicalize().unwrap();
        let workflow_root = root.join("wf-a");
        std::fs::create_dir_all(&workflow_root).unwrap();
        std::fs::write(workflow_root.join("WORKFLOW.md"), "noop").unwrap();
        let paths = AgentPaths::with_workflow(
            root.clone(),
            workflow_root.clone(),
            root.join("workflows")
                .join(workflow_key(&workflow_root.join("WORKFLOW.md"))),
        );
        let tracker = Arc::new(FileTracker::new(
            workflow_root.join("issues"),
            vec!["todo".to_string()],
            vec!["done".to_string()],
        ));
        let prompt = PromptRenderer::load(&workflow_root.join("WORKFLOW.md")).unwrap();
        let effective_cfg = EffectiveLoopConfig::resolve(&cfg, &test_config_frontmatter());
