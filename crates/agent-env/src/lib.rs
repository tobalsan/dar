...
fn root_lock(root: &Path) -> Arc<Mutex<()>> {
    ...
    LOCKS
        ...
        .lock()
        .expect("agent env root lock registry poisoned")
        .entry(root.to_path_buf())
        .or_insert_with(|| Arc::new(Mutex::new(())))
        .clone()
}

/// Process-local, root-scoped read-through view of agent environment.
pub struct RootAgentEnv {
    root: PathBuf,
...
impl RootAgentEnv {
    fn new(root: &Path) -> Self {
        ...
        let _guard = lock.lock().expect("agent env root lock poisoned");
        let values = read_stable(root).unwrap_or_default();
        Self {
    ...
    fn refresh(&self) {
        let lock = root_lock(&self.root);
        let _guard = lock.lock().expect("agent env root lock poisoned");
        let Ok(values) = read_stable(&self.root) else {
            return;
        };
        let next = fingerprint(&values);
        let mut state = self.state.lock().expect("agent env provider poisoned");
        if state.fingerprint == Some(next) {
            return;
        }
        for value in loaded_agent_env_values(&self.root) {
            if !value.is_empty() {
                state.secrets.insert(value);
            }
        }
        if reload_agent_env_values(&self.root, values).is_ok() {
            for value in loaded_agent_env_values(&self.root) {
                if !value.is_empty() {
                    state.secrets.insert(value);
                }
            }
            state.fingerprint = Some(next);
        }
    }
}

impl AgentEnv for RootAgentEnv {
    ...
    fn secret_values(&self) -> Vec<String> {
        ...
        self.state
            .lock()
            .expect("agent env provider poisoned")
            .secrets
            .iter()
            .cloned()
            .collect()
...
/// Return shared process-local provider for one agent root.
pub fn provider(root: &Path) -> Arc<dyn AgentEnv> {
    static PROVIDERS: OnceLock<Mutex<std::collections::HashMap<PathBuf, Arc<RootAgentEnv>>>> =
        OnceLock::new();
    let mut providers = PROVIDERS
        .get_or_init(|| Mutex::new(std::collections::HashMap::new()))
        .lock()
        .expect("agent env provider registry poisoned");
    providers
        .entry(root.to_path_buf())
        .or_insert_with(|| Arc::new(RootAgentEnv::new(root)))
        .clone()
}
...
mod tests {
    ...
    #[test]
    fn provider_refreshes_rotation_keeps_last_good_and_honors_deletion() {
        let _guard = ENV_LOCK.get_or_init(|| Mutex::new(())).lock().unwrap();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join(".env");
        std::env::remove_var("PROVIDER_SECRET");
        std::fs::write(&path, "PROVIDER_SECRET=old\n").unwrap();
        load_agent_env(dir.path()).unwrap();
        let env = super::provider(dir.path());
        assert_eq!(env.get("PROVIDER_SECRET").as_deref(), Some("old"));

        std::fs::write(&path, "PROVIDER_SECRET=new\n").unwrap();
        assert_eq!(env.get("PROVIDER_SECRET").as_deref(), Some("new"));
        let secrets = env.secret_values();
        assert!(secrets.contains(&"old".to_string()));
        assert!(secrets.contains(&"new".to_string()));

        std::fs::write(&path, "BROKEN\n").unwrap();
        assert_eq!(env.get("PROVIDER_SECRET").as_deref(), Some("new"));
        std::fs::remove_file(&path).unwrap();
        assert_eq!(env.get("PROVIDER_SECRET"), None);
        std::env::remove_var("PROVIDER_SECRET");
    }

    #[test]
    fn provider_preserves_genuine_process_env_precedence() {
        let _guard = ENV_LOCK.get_or_init(|| Mutex::new(())).lock().unwrap();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join(".env");
        std::env::set_var("PROVIDER_EXTERNAL", "external");
        std::fs::write(&path, "PROVIDER_EXTERNAL=file-one\n").unwrap();
        load_agent_env(dir.path()).unwrap();
        let env = super::provider(dir.path());
        std::fs::write(&path, "PROVIDER_EXTERNAL=file-two\n").unwrap();
        assert_eq!(env.get("PROVIDER_EXTERNAL").as_deref(), Some("external"));
        std::env::remove_var("PROVIDER_EXTERNAL");
    }
