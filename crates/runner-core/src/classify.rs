...
pub fn classify_protocol_line(stream: &str, text: &str) -> ProtocolLine {
    ...
    if let Ok(value) = serde_json::from_str::<serde_json::Value>(text) {
        ...
        ProtocolLine {
            row_type: normalize_log_row(stream, text),
            text: text.to_string(),
            detail: String::new(),
        }
    }
...
fn classify_message_end(v: &serde_json::Value) -> ProtocolLine {
    ...
    match role.as_deref() {
        Some("assistant") => ProtocolLine {
            row_type: "assistant",
            text,
            detail: String::new(),
        },
        Some("user") => ProtocolLine {
            row_type: "user",
            text,
            detail: String::new(),
        },
            ...
            match status {
                ...
                "error" => {
                    let msg = state
                        ...
                        .or_else(|| {
                            state.and_then(|s| s.get("error").and_then(serde_json::Value::as_str))
                        })
                        .unwrap_or("error");
                    ProtocolLine {
                        row_type: "error",
...
fn classify_jsonrpc_response(v: &serde_json::Value) -> ProtocolLine {
...
fn classify_jsonrpc_error_notification(
    method: &str,
    params_obj: Option<&serde_json::Map<String, serde_json::Value>>,
) -> Option<ProtocolLine> {
    if method == "error" {
        let msg = params_obj
...
fn classify_jsonrpc_item(
    ...
    if item_type == "userMessage" {
        let item_val = item.map(|i| serde_json::Value::Object(i.clone()));
        let text =
            join_text_entries(item_val.as_ref().and_then(|v| v.get("content"))).unwrap_or_default();
        if text.is_empty() {
            return Some(hidden());
        }
        return Some(ProtocolLine {
...
fn classify_jsonrpc_tool_call(
    ...
    let (text, detail) = if item_type == "commandExecution" {
        ...
        let label = item
            .and_then(|i| i.get("tool").and_then(|t| t.as_str()))
            .unwrap_or(item_type)
            .to_string();
        let detail = item
            .and_then(|i| i.get("prompt").and_then(|p| p.as_str()))
            .unwrap_or("")
            .to_string();
        (strip_ansi(&label), strip_ansi(&detail))
    } else {
        let label = match item_type {
            "fileChange" => item
                ...
                .to_string(),
            "webSearch" => item
                .and_then(|i| i.get("query").and_then(|q| q.as_str()))
                .unwrap_or(item_type)
                .to_string(),
            _ => item
        ...
        let detail = item
            .and_then(|i| i.get("output").or_else(|| i.get("result")))
            .map(|v| {
                if v.is_string() {
...
fn classify_jsonrpc_line(v: &serde_json::Value) -> ProtocolLine {
    ...
    if method.is_none() {
        return classify_jsonrpc_response(v);
    }
    ...
    if let Some(pl) = classify_jsonrpc_error_notification(method, params_obj) {
        return pl;
    }

    if let Some(pl) = classify_jsonrpc_item(item_type, item, method) {
        return pl;
    }
    ...
    if tool_types.contains(&item_type) {
        return classify_jsonrpc_tool_call(item_type, item);
    }
...
mod tests {
    ...
    fn jsonrpc_command_execution_exit_zero_no_suffix() {
        ...
        assert_eq!(pl.row_type, "tool_call");
        assert!(
            !pl.detail.contains("[exit"),
            "detail should not have exit suffix: {}",
            pl.detail
        );
    }
    ...
    fn pi_tool_execution_start_and_update_are_hidden() {
        ...
        let update =
            r#"{"type":"tool_execution_update","toolCallId":"c1","partialResult":"total 4"}"#;
