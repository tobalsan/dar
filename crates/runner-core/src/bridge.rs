...
pub fn codex_mcp_bridge_args(bridge: &HostToolBridge) -> Vec<OsString> {
    ...
    vec![
        OsString::from("-c"),
        OsString::from(format!("mcp_servers.{BRIDGE_SERVER_NAME}.command={command_toml}")),
        OsString::from("-c"),
...
mod tests {
    ...
    fn pi_mcp_config_args_writes_config_and_returns_flag_and_env() {
        ...
        assert_eq!(written["mcpServers"][BRIDGE_SERVER_NAME]["command"], "/opt/agentropy");
        assert_eq!(
            written["mcpServers"][BRIDGE_SERVER_NAME]["args"],
            json!(["__mcp-bridge", "--dir", "/tmp/agent"])
        );
        assert_eq!(written["mcpServers"][BRIDGE_SERVER_NAME]["lifecycle"], "eager");
    ...
    fn codex_mcp_bridge_args_render_toml_command_and_args() {
        ...
        assert_eq!(rendered[1], "mcp_servers.agentropy.command=\"/opt/agentropy\"");
